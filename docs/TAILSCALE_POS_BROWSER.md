...
Адреси **`100.x.x.x`** існують **тільки всередині Tailscale**. Звичайний телефон або ПК без клієнта Tailscale **не зможе** відкрити `http://100.95.64.38:3080`.

**Що зробити:** установи **Tailscale** на той самий пристрій, з якого відкриваєш браузер (ПК / ноут / телефон — для Android/iOS є додаток Tailscale). Увійди в **той самий акаунт**, що й на касі. Потім знову `http://100.95.64.38:3080`.
...
```powershell
Test-NetConnection -ComputerName 100.95.64.38 -Port 3080
```
...
```powershell
.\diagnose-tailscale-from-home.ps1 -BarIp 100.95.64.38
```
...
Якщо в **Access controls** є кастомний `acl.hujson`, який **забороняє** трафік між нодами — дозволь хоча б `TCP 3080` між твоїм ПК і касою (або тимчасово спрости ACL до дефолту для перевірки).
