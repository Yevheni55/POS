...
import { createPaymentSchema, changePaymentMethodSchema } from '../schemas/payments.js';
...
router.post('/:id/change-method', mgr, validate(changePaymentMethodSchema), asyncRoute(async (req, res) => {
  const paymentId = Number.parseInt(req.params.id, 10);
  if (!Number.isFinite(paymentId)) {
    return res.status(400).json({ error: 'Neplatne ID platby' });
  }
