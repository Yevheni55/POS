...
(function(){var searchTimer=null;document.getElementById('searchInput').addEventListener('input',function(e){searchQuery=e.target.value.toLowerCase().trim();_updateSearchClearBtn();clearTimeout(searchTimer);searchTimer=setTimeout(function(){renderProducts()},200)})})();

// === Custom search keyboard ===
// Nahradza nativnu Android KB ktora zakryvala pol products grid. Otvori
// sa tap-om na searchInput (alebo na search-wrap), zatvorenie cez tap
// mimo alebo cez X button. 3 vrstvy (abc/dia/123) ako note-kb. Live
// filter cez existujuci input event listener.
var _searchKbLayer = 'abc';
var _searchKbOpen = false;

function _updateSearchClearBtn(){
  var btn = document.getElementById('searchClearBtn');
  var input = document.getElementById('searchInput');
  if (!btn || !input) return;
  if ((input.value || '').length > 0) btn.removeAttribute('hidden');
  else btn.setAttribute('hidden', '');
}

function openSearchKeyboard(){
  if (_searchKbOpen) return;
  _searchKbOpen = true;
  _searchKbLayer = 'abc';
  _renderSearchKeyboard();
  var kb = document.getElementById('searchKeyboard');
  if (kb) kb.removeAttribute('hidden');
  // Pridame body class aby CSS mohol upravit padding products grid pre kb
  document.body.classList.add('search-kb-open');
}
...
function closeSearchKeyboard(){
  ...
  document.body.classList.remove('search-kb-open');
}

function _renderSearchKeyboard(){
  var host = document.getElementById('searchKeyboard');
  if (!host) return;
  var rows;
  if (_searchKbLayer === 'abc'){
    rows = [
      ['q','w','e','r','t','y','u','i','o','p'],
      ['a','s','d','f','g','h','j','k','l'],
      ['z','x','c','v','b','n','m'],
    ];
  } else if (_searchKbLayer === 'dia'){
    rows = [
      ['á','č','ď','é','í','ľ','ĺ','ň'],
      ['ó','ô','ŕ','š','ť','ú','ý','ž'],
    ];
  ...
  for (var r = 0; r < rows.length; r++){
    html += '<div class="search-kb-row">';
    for (var c = 0; c < rows[r].length; c++){
      var k = rows[r][c];
      var label = k === ' ' ? '␣' : k;
      html += '<button type="button" class="search-kb-key" data-skb-key="' + k + '">' + label + '</button>';
    }
    html += '</div>';
  }
  // Akcie: toggle vrstvy + space + backspace + close
  html += '<div class="search-kb-row search-kb-actions">';
  html += '<button type="button" class="search-kb-key is-mod" data-skb-action="toggle-dia">'
    + (_searchKbLayer === 'dia' ? 'abc' : 'áž') + '</button>';
  html += '<button type="button" class="search-kb-key is-mod" data-skb-action="toggle-123">'
    + (_searchKbLayer === '123' ? 'abc' : '123') + '</button>';
  html += '<button type="button" class="search-kb-key is-space" data-skb-key=" ">medzera</button>';
  html += '<button type="button" class="search-kb-key is-mod" data-skb-action="backspace">⌫</button>';
  html += '<button type="button" class="search-kb-key is-close" data-skb-action="close">Hotovo</button>';
  html += '</div>';
  host.innerHTML = html;
}

function _searchKbAppend(k){
  var input = document.getElementById('searchInput');
  if (!input) return;
  if ((input.value || '').length >= 50) return; // limit search query length
  input.value = (input.value || '') + k;
  input.dispatchEvent(new Event('input', { bubbles: true }));
}

function _searchKbBackspace(){
  var input = document.getElementById('searchInput');
  if (!input) return;
...
// Click handlers — delegated. Klavesy, modifiers, otvorenie cez search-wrap.
document.addEventListener('click', function(e){
  // Klavesa stlacena
  var key = e.target && e.target.closest && e.target.closest('.search-kb-key');
  if (key){
    var act = key.dataset.skbAction;
    if (act === 'backspace'){ _searchKbBackspace(); return; }
    if (act === 'toggle-dia'){ _searchKbLayer = _searchKbLayer === 'dia' ? 'abc' : 'dia'; _renderSearchKeyboard(); return; }
    if (act === 'toggle-123'){ _searchKbLayer = _searchKbLayer === '123' ? 'abc' : '123'; _renderSearchKeyboard(); return; }
    if (act === 'close'){ closeSearchKeyboard(); return; }
    var k = key.dataset.skbKey;
    if (k != null) _searchKbAppend(k);
    return;
  }
  // Tap mimo search-wrap + keyboard → close ak je otvorena
  if (_searchKbOpen){
    var inSearch = e.target.closest && (e.target.closest('.search-wrap') || e.target.closest('#searchKeyboard'));
    if (!inSearch) closeSearchKeyboard();
  }
});

// Open keyboard pri click/focus na searchInput alebo search-wrap.
document.addEventListener('click', function(e){
  var wrap = e.target && e.target.closest && e.target.closest('.search-wrap');
  if (wrap && !e.target.closest('#searchClearBtn')) {
    openSearchKeyboard();
  }
});

// Clear button — zmaz query a zatvor kb.
document.addEventListener('click', function(e){
  var clr = e.target && e.target.closest && e.target.closest('#searchClearBtn');
  if (!clr) return;
  var input = document.getElementById('searchInput');
  if (input){
    input.value = '';
...
// (KB sa schova prirodzene ked productsPanel stratí .active class pri
// switchView('tables') — display:none na rodicovi pochova aj #searchKeyboard.
// Nepoužívame patch nad switchView lebo function declaration je hoisted
// a window.* nahradzanie by nezmenilo lokalne bindings.)
